...
export const WhatsappButton: React.FC = () => {
  ...
  return (
    <div className={`fixed bottom-12 right-6 z-50 transition-all duration-500 ease-out ${visible ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
      <a href={`https://wa.me/${phone}?text=${message}`} target='_blank' rel='noopener noreferrer' aria-label='Fale com Daniel no WhatsApp'>
        <div className='relative'>
          ...
          <div className='w-14 h-14 rounded-full flex items-center justify-center shadow-md' style={{ backgroundColor: COR_DESTAQUE }}>
            <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 2 30 30' fill={COR_WPP_BUTTON} className='w-10 h-10'>
